...
def generate_plan(user):
    """
    Credit: Doc-string generated by GPT-4
    ...
    Querying Strategy:
    ------------------
    - Only 'active' cases are considered (`status='active'`).
    - All diseases found in each case is collected per orchard.
    - The algorithm takes a conservative approach, always selecting the highest possible risk values.
    - For each orchard, the highest severity and spreadability values across these diseases are used.
    - The highest variety and location susceptibility scores out of all diseases are selected.
    ...
    -------------------------------------
    - severity : float
        Maximum disease severity across active cases.

    - spreadability : float
        Maximum disease spreadability across active cases.
...
def calc_last_check_score(orchard):
    ...
    """
    today = date.today()
    last_check_date = None # Record.objects.filter(orchardID=orchard).order_by('-recordedAt').first()
